.PHONY: specs world generate-world serve sync install stop start restart logs

SEED ?=
DB ?= data/catalog.db
WORLD_CONFIG ?= config/world-generation.v2.yaml
DEPLOY_HOST ?= uptick
LOG_LINES ?= 200
...
generate-world:
	@if [ -z "$(SEED)" ]; then echo "usage: make world SEED=1 [DB=data/catalog.db] [WORLD_CONFIG=config/world-generation.v2.yaml]"; exit 2; fi
