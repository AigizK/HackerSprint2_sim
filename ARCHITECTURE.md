...
- реальное время синхронизируется перед действием, а `/time/advance` применяет
  ...
  подходящую ошибку публичного лога в этом интервале, затем атомарно применяет
  события до всей её временной группы. Необязательный непустой уникальный
  `stop_when.error_codes` ограничивает matching точными кодами с OR-семантикой;
  список сортируется перед формированием идемпотентного payload. Отметка
  последнего реального запроса хранится в SQLite и восстанавливается после
  рестарта;
