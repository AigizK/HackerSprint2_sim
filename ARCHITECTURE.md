...
`WorldEvaluator` запускается сразу после генератора и строит закрытый benchmark с полным знанием расписания. Версия `world-evaluator.v1` определяет детерминированные покупательские намерения, минимальную инфраструктуру для безошибочного visitor journey, устраняет заранее известные page bugs и не выполняет необязательные действия, уменьшающие итоговую операционную прибыль. Backend нельзя масштабировать ниже одного instance, даже если обслуживание мира убыточно.

Benchmark содержит максимальные операционную прибыль, выручку и баланс, минимальные расходы на серверы, количество покупок и oracle plan. Завершённый run получает score только при visitor availability не ниже 95%; synthetic probes в SLO не входят. Для прошедшего SLO run score нормализует операционную прибыль без стартового капитала относительно benchmark. Минимальное реальное время равно количеству запросов в oracle plan, умноженному на `10s`; длительность симуляционного мира в этот показатель не входит. Evaluation хранится JSON-полем в `worlds`.
