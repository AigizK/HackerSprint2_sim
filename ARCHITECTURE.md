...
`make calibrate` генерирует десять уменьшенных месячных sample-миров для seed 1–10 и прогоняет их через FSM с контрольной плохой стратегией из `config/calibration.v1.yaml`: шесть backend instances, отсутствие fixes и суточные шаги времени. Sample target задаётся отдельно (`1000`), чтобы калибровочная проверка оставалась быстрой; production target равен 100 000 событий при защитном лимите 10 000 000. Дополнительно выводятся выбранный период, дневной максимум посетителей и максимальное изменение между соседними днями.
